...
function getSchemaPrimitiveType(schema) {
  var parts, type = schema, typeParts;
  ...
  if (typeof schema === 'string') {
    ...
    if (typeParts.length > 0 && typeParts[1]) {
      type = typeParts[1];
    }
  }
...
DefinitionsElement.prototype.addChild = function(child) {
  ...
  if (child instanceof TypesElement) {
    ...
    if (!self.portTypes[child.$name]) {
      self.portTypes[child.$name] = child;
    } else {
      self.portTypes[child.$name].children = self.portTypes[child.$name].children.concat(child.children);
    }
  }
  else if (child instanceof BindingElement) {
    if (child.transport === 'http://schemas.xmlsoap.org/soap/http' ||
      child.transport === 'http://www.w3.org/2003/05/soap/bindings/HTTP/')
      if (!self.bindings[child.$name]) {
        self.bindings[child.$name] = child;  
      } else {
        self.bindings[child.$name].children = self.bindings[child.$name].children.concat(child.children);
      }
  }
 ...
 * property and returns it in a underscore ('#') separated String (using '^' as default
...
MessageElement.prototype._getNestedLookupTypeString = function (element) {
  ...
  if (element.hasOwnProperty('$type') && typeof element.$type === 'string') {
    if (excluded.indexOf(element.$type.split(':')[0]) === -1) {
      resolvedType += ('#' + element.$type + '#' + element.$name);
    }
  }
  ...
  if (element.children.length > 0) {
    ...
    element.children.forEach(function (child) {
      var resolvedChildType = self._getNestedLookupTypeString(child).replace(/\^#/, '');

      if (resolvedChildType && typeof resolvedChildType === 'string') {
        resolvedType += ('#' + resolvedChildType);
      }
    });
  }
...
ElementElement.prototype.description = function(definitions, xmlns) {
  ...
  if (type) {
    ...
    /*TODO JB this doesn't handle types that have a element and a simple type (e.g. size)
     I think it needs to use simpletype if it's an attribute */
    var typeName = type.name,
      ns = xmlns && xmlns[type.namespace] || definitions.xmlns[type.namespace],
      schema = definitions.schemas[ns],
      typeElement = schema && ( schema.complexTypes[typeName] || schema.elements[typeName] || schema.types[typeName]);
    ...
    if (typeElement && !(typeName in Primitives)) {
      ...
      if (!(typeName in definitions.descriptions.types)) {
        ...
        var description = _.cloneDeep(typeElement.description(definitions, xmlns));
        ...
        if (this.$ref) {
          element = _.clone(elem);
          element.arrayHint = arrayHint;
        }
        else {
          element[name] = elem;
        }
        ...
        if (this.$ref) {
          element = _.clone(definitions.descriptions.types[typeName]);
          element.arrayHint = arrayHint;
        }
        else {
...
WSDL.prototype.ignoredNamespaces = ['tns', 'targetNamespace', 'typedNamespace', 'xt'];
...
WSDL.prototype._initializeOptions = function (options) {
  ...
  } else if (ignoredNamespaces && Array.isArray(ignoredNamespaces)) {
    this.options.ignoredNamespaces = this.ignoredNamespaces = this.ignoredNamespaces.concat(ignoredNamespaces);
  } else {
...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  p.onclosetag = function(nsName) {
    ...
    if (getSchemaPrimitiveType(cur.schema) === 'string') {
    ...
    if ((top.schema && top.schema.arrayHint) || (topSchema && topSchema[name + '[]'])) {
      if (!topObject[name])
        topObject[name] = [];
  ...
  p.ontext = function(text) {
    ...
    if (top && top.schema) {
      schemaType = getSchemaPrimitiveType(top.schema);
    }

    var intTypes = ['byte', 'decimal', 'int', 'integer', 'long', 'negativeInteger', 'nonNegativeInteger',
      'nonPositiveInteger', 'positiveInteger', 'short', 'unsignedLong', 'unsignedInt', 'unsignedShort', 'unsignedByte'];
    var decimalTypes = ['decimal'];
    var boolTypes = ['bool', 'boolean'];
    var dateTypes = ['date', 'dateTime'];

    if (intTypes.indexOf(name) !== -1 || intTypes.indexOf(schemaType) !== -1) {
      value = parseInt(text, 10);
    } else if (decimalTypes.indexOf(name) !== -1 || decimalTypes.indexOf(schemaType) !== -1) {
      value = parseFloat(text);
    } else if (boolTypes.indexOf(name) !== -1 || boolTypes.indexOf(schemaType) !== -1) {
      value = text.toLowerCase() === 'true' || text === '1';
    } else if (dateTypes.indexOf(name) !== -1 || dateTypes.indexOf(schemaType) !== -1) {
      value = new Date(text);
    } else {
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, isFirst, xmlnsAttr, parameterTypeObject, ancestorXmlns) {
  ...
  if (Array.isArray(obj)) {
    for (var i = 0, item; item = obj[i]; i++) {
      ...
      if (this.options.ignoredNamespaces.indexOf(correctOuterNamespace.replace(':', '')) > -1) {
        correctOuterNamespace = ns;
      }
    ...
    for (name in obj) {
      ...
      if (this.ignoredNamespaces.indexOf(ns.replace(':', '')) !== -1) {
        ns = namespace + ':';
      }
